...
class Dao(object):
    ...
    def find_all(self,key):
        c = self._conn.cursor()
        c.execute('SELECT * FROM {} ORDER BY {} '.format(self._table_name, key))
        return orm(c, self._dto_type)
    ...
    def update(self, set_values, cond):
        ...
        #Warning - In course site at following line there are 2 extra '()'
        stmt = 'UPDATE {} SET {} WHERE {}'\
                .format(self._table_name, ', '.join([set + '=?' for set in set_column_names]),
